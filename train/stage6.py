"""Stage 6 — HTML table retargeting, aimed squarely at TEDS.
...
Design:
  ...
  * Table-bearing rows upweighted 2.0; a broad protective mix of every other domain
    is kept — every prior narrow-focus stage damaged unrelated suites, so the
    protective dose is not optional.
...
CKPT_DIR = os.path.join(BASE, "train", "checkpoints", "stage6")
...
DOMAIN_WEIGHT = {
    "doc_markdown": 0.12,           # non-table doc pages: keep the format warm
...
def main():
    ...
    out = os.path.join(BASE, "train", "adapters", "eyoun-s6")
